...
use crate::streams::{services::events::WitnessedEvent, configs::LocalNetworkConfiguration};
use std::sync::{Arc, Mutex};
use super::{StreamsGossipHandler, StreamsGossip};
...
pub async fn test_self_message()
{
    ...
    task_manager.spawn_handle().spawn("Test", None,service.start(spawn_handle, self_addr.clone(), vec![self_addr.clone()], mock_handler.clone()));
    task_manager.spawn_handle().spawn("Test2", None,mock_peer_service.start(peer_spawn_handle, peer_mock_addr.clone(), vec![], mock_handler.clone()));
    ...
    tokio::time::sleep(Duration::from_millis(1000)).await;
    streams_gossip.publish(IdentTopic::new("WitnessedEvent"),bincode::serialize(&witnessed_event).unwrap()).await;
