...
async fn witness_events(validator_addr: String, limit: u32, increase_factor: u32) {
	...
	while let Some(response) = stream.message().await.unwrap() {
		...
		// if the average number of events drop and the total number of events reached the number of
		// the last batch send new batch of txs to make sure we have more events in the tx pool than
		// what the block can contain, this will be a balnaced approach of witnessing lots of events
		// and also let the validator focus more on processing gossiped witnessed events from his
		// peers
		else if (new_average < average_events_per_block) &&
			(new_average != 0) &&
			...
			(num_events <= limit)
		{
			...
			println!(
				"\n🩸 Average number of events dropped, sending {} events",
				new_num_events - num_events
			);
			let validator_addr_clone = validator_addr.clone();
			tokio::spawn(async move {
				send_events(num_events, new_num_events, validator_addr_clone.clone()).await
			});
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
	...
	println!(
		"Connected to {}, Increase factor:{}, Max events:{}",
		validator_addr, increase_factor, limit
	);
	send_events(0, INIT_NUM, validator_addr.clone()).await;
	let _ = tokio::spawn(async move {
		witness_events(validator_addr, limit, increase_factor).await;
	})
	.await;
	Ok(())
}
