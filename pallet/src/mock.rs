...
use frame_support::{traits::{ConstU16, ConstU32, ConstU64}, BoundedVec, once_cell::sync::Lazy};
...
impl pallet_validated_streams::Config for Test {
	...
	fn authorities() -> frame_support::BoundedVec<Self::VSAuthorityId, Self::VSMaxAuthorities> {
        ...
        { 
            ...
            let id = AuraId::from_slice(PAIRS.lock().unwrap().get(i).unwrap().1.as_slice()).unwrap();
            authorities.push(id);
        }
